...
import com.verner.healthgateway.presentation.component.stepsrecord.StepsRecordRow
import java.time.ZonedDateTime
import java.util.UUID


@Composable
fun StepsRecordScreen(
  permissions: Set<String>,
  permissionsGranted: Boolean,
  recordsList: List<StepsRecordData>,
  uiState: StepsRecordViewModel.UiState,
  onImportClick: () -> Unit = {},
  ...
  LaunchedEffect(uiState) {
    ...
    if (uiState is StepsRecordViewModel.UiState.Uninitialized) {
      onPermissionsResult()
    }
    ...
    if (uiState is StepsRecordViewModel.UiState.Error && errorId.value != uiState.uuid) {
      onError(uiState.exception)
      errorId.value = uiState.uuid
    }
  }

  if (uiState != StepsRecordViewModel.UiState.Uninitialized) {
    LazyColumn(
